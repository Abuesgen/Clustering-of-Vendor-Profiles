...
EMBEDDING_TYPES = [ProfileEmbedder.WORD, ProfileEmbedder.FLAIR, ProfileEmbedder.TRANSFORMER, ProfileEmbedder.MODEL]
...
def cmd_cluster_profiles(  # pylint: disable=too-many-locals
    ...
    profile_embedder: ProfileEmbedder = get_profile_embedder(
        model, embeddings, tf_idf, contextualized, embeddings_type
    )
...
def write_results(clustered: pd.DataFrame, visualizations_path: Path, output_dir: Path, score: float):
    ...
    for _, row in tqdm(
        ...
        cluster_path = output_dir / str(row["cluster"]).replace("/", "_").strip()
