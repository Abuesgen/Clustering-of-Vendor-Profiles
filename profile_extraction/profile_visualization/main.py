...
@click.option("--model-path", type=click.Path(dir_okay=False, file_okay=True, readable=True), required=True)
def cmd_profiles_to_html(input_json, output_dir, model_path):
    """
    ...
    message_str_to_doc = preprocess_messages(data, model)

    for profile in tqdm(data.profiles, desc="Creating visualizations"):
        create_profile_visualization(message_str_to_doc, output_dir, profile)
