...
COLORS = {"LOC": "#5C8AA8", "PAYM": "#FB2050", "PER": "#CC8033", "MONEY": "#8CB500", "PROD": "#9966CC"}


def create_html(nlp: Dict[str, List[Sentence]], profile: Profile):
    """
    ...
    with doc:
        ...
        with div(cls="container"):
            ...
            add_products(nlp, profile, summary)
...
def add_products(nlp: Dict[str, List[Sentence]], profile, summary):
    """
    ...
    with div(id="products", cls="row"):
        with div(cls="col-sm-12", id="product_accordion"):
            ...
            for product in summary.products:
                add_product(nlp, product, profile, product_id)
                product_id += 1
...
def add_product(nlp: Dict[str, List[Sentence]], product, profile, prod_id):
    """
    ...
    with div(cls="card"):
        ...
        with div(id=f"prod_{prod_id}", cls="collapse", data_parent="#product_accordion"):
            with div(cls="card-body"):
                add_messages(profile, product.message_ids, nlp)
...
def add_messages(profile: Profile, message_ids: Iterable[int], nlp: Dict[str, List[Sentence]]):
    """
    ...
    with table(cls="table table-responsive table-striped", style="max-height: 50vh"):
        ...
        with tbody():
            for message_str, ids in message_dict.items():
                add_message(nlp, message_str, ids)


def add_message(nlp: Dict[str, List[Sentence]], message: str, ids: List[int]):
    """
                ...
                for sentence in sentences:
                    ...
                    if len(entities) > 0:
                        ...
                        for entity in entities:
                            span(sentence.to_original_text()[end_idx : entity.start_pos])
                            ...
                            with mark(
                                ...
                                marked_entity.add(sentence.to_original_text()[entity.start_pos : entity.end_pos])
                                with span(
                        ...
                        span(sentence.to_original_text())
                    br()
...
def preprocess_messages(profiles: ProfileCollection, model: SequenceTagger) -> Dict[str, List[Sentence]]:
    """
    Process all messages using flair
    :param profiles:  All Profiles to process
    :param model: flair SequenceTagger
    :return: Mapping of string to processed sentences
    """

    message_strs: Set[str] = set()
    for profile in profiles:
        for profile_comp in profile.components:
            if isinstance(profile_comp, BaseComponent):
                message_strs.add(ensure_not_none(profile_comp.message.message))
    message_strs_list: List[str] = list(message_strs)

    message_sentences = [
        [
            Sentence(message_line, use_tokenizer=SegtokTokenizer())
            for message_line in message_str.split("\n")
            if len(message_line.strip()) > 0
        ]
        for message_str in message_strs_list
    ]

    for sentences in tqdm(message_sentences, desc="Predicting NEs"):
        model.predict(sentences)

    message_str_to_doc = dict(zip(message_strs_list, message_sentences))
    return message_str_to_doc


def create_profile_visualization(nlp: Dict[str, List[Sentence]], output_dir, profile):
    """
