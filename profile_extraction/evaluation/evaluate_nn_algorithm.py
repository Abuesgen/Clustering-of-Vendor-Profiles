...
class JsonDatasetEntry(BaseModel):
    ...
    data: str
    label: List[Union[List[Union[str, int]], JsonLabel]] = []
    relations: List[Union[List[Union[str, int]], JsonRelationLabel]] = []

    @validator("label")
    def parse_labels(cls, value):  # pylint: disable=no-self-argument,no-self-use
        """
        Converts doccanos list style labels into DTOS
    ...
    @validator("relations")
    def parse_relation_labels(cls, value):  # pylint: disable=no-self-argument,no-self-use
        """
...
def count_correct_prod_price_relations(datapoint: JsonDatasetEntry):
    ...
    """
    prods = [prod for prod in datapoint.label if isinstance(prod, JsonLabel) and prod.label == "PROD"]
    money = [money for money in datapoint.label if isinstance(money, JsonLabel) and money.label == "MONEY"]
