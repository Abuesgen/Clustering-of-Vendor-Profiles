...
def score_pipeline():
    ...
    nlp = create_pipeline(
        "/home/lars/Projects/data-2022-profile-extraction/rule_based_pipeline_resources/patterns.jsonl"
    )
    scorer = Scorer(default_pipeline=["entity_ruler"])

    doccano_annotation = list(srsly.read_jsonl("/home/lars/Projects/data-2022-profile-extraction/tmp/test.jsonl"))
    dc = DoccanoSpacyConverter(nlp, labels=["PROD", "PAYM", "PER", "MONEY", "LOC"])
