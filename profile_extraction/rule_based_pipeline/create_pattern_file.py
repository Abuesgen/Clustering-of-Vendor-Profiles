...
def create_patterns(train_sample_it: Iterable[Dict]):
    ...
    phrases: Dict[str, Set[Tuple[str, ...]]] = {"PROD": set(), "PER": set(), "PAYM": set(), "LOC": set()}
    ...
    phrase_pattern = lambda phrase: [{"LOWER": p} if i < 2 else {"LOWER": p, "OP": "?"} for i, p in enumerate(phrase)]

    for label, phrase_set in phrases.items():
        for phrase in phrase_set:
            patterns += [{"label": label, "pattern": phrase_pattern(phrase)}]
